...
public class Activator {
	...
	public void start() throws Exception {
		...
		// scFacadeReg =
		// context.registerService(ISnmpClientFacade.class.getName(), facade,
		// props);
		if (getBooleanProperty(ISnmpTrapD.TRAPD_ENABLE)) {
			trapD = new Snmp4JTrapD();
			trapD.setListenAddress("udp:0.0.0.0/"
					+ getIntProperty(ISnmpTrapD.TRAPD_PORT, 162));
			trapD.setPoolSize(getIntProperty(ISnmpTrapD.TRAPD_THREAD, 10));
			trapD.start();
			props.put("uid", "Snmp:SnmpTrapd");
			// trapDReg = context.registerService(ISnmpTrapD.class.getName(),
			// trapD, props);
		}
	}
